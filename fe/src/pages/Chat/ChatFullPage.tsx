...
export default function ChatFullPage() {
    ...
    const handleSendMessage = async (text: string) => {
        ...
        const meMsg: MessageItemProps = { content: text, sender: "me", timestamp: new Date() };
        setInputValue("");
        ...
        try {
            ...
            setMessages((prev) => [
                ...prev,
                { content: "Xin lỗi, có lỗi xảy ra. Vui lòng thử lại sau.", sender: "model", timestamp: new Date() },
            ]);
        }
        setIsLoading(false);
    };
    ...
    return (
        <div className="h-screen min-h-0 bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950 flex flex-col overflow-hidden">
            {/* Header */}
            <div className="bg-gradient-to-r from-yellow-500 to-yellow-600 px-8 py-6 shadow-lg flex-shrink-0">
                <div className="max-w-5xl mx-auto flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="font-bold text-white text-2xl">Gia sư Tư tưởng Hồ Chí Minh</h1>
            ...
            <div className="flex-1 min-h-0 flex flex-col">
                ...
                <div
                    ...
                    onWheel={(e) => { e.preventDefault(); e.stopPropagation(); messagesContainerRef.current!.scrollTop += e.deltaY; }}
                ...
                >
                    <div className="max-w-4xl mx-auto space-y-6">
                        {messages.length === 1 && messages[0].sender === "model" && (
                            <div className="mb-8">
                                <h2 className="text-zinc-400 text-lg mb-4 text-center">Câu hỏi gợi ý</h2>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {suggestions.map((s, i) => (
                                        ...
                                        >
                                            ...
                                            <div className="text-zinc-300 group-hover:text-yellow-400 transition-colors">{s.title}</div>
                                        </button>
                                    ))}
                                </div>
                            </div>
            ...
            <div className="bg-zinc-900 border-t border-zinc-800 px-8 py-6 flex-shrink-0">
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="text-sm text-zinc-500 mt-3 text-center">Nhấn Enter để gửi • Shift + Enter để xuống dòng</div>
                </div>
            </div>
...
const MessageItem = ({ data }: { data: MessageItemProps }) => {
    ...
    return (
        <div className={`flex gap-4 ${data.sender === "me" ? "justify-end" : "justify-start"}`}>
            {data.sender === "model" && (
            ...
            <div
                className={`max-w-[70%] px-6 py-4 rounded-3xl ${data.sender === "me" ? "bg-yellow-600 text-white rounded-tr-md" : "bg-zinc-800 text-zinc-200 rounded-tl-md"
                    }`}
                style={{ wordWrap: "break-word", overflowWrap: "break-word", wordBreak: "break-word" }}
            >
                <div className="message-content text-base leading-relaxed" dangerouslySetInnerHTML={{ __html: parsedContent }} />
                {data.timestamp && (

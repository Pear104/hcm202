  ...
  return (
    <>
      ...
      >
        <div
          id="scroll-title"
          className="sticky top-[4vh] font-semibold text-[6vh] uppercase overflow-x-scroll w-screen"
        >
          <div className="px-[4vw]">
            Thực tiễn phát huy dân chủ xã hội chủ nghĩa
            <br />ở
            <span className="text-yellow-400 font-bold text-[14vh] mx-[2vw]">
              Việt Nam
            </span>{" "}
            hiện nay
          </div>
          <div
            id="scroll-wrapper"
            className="flex gap-[4vw] w-[136vw] mx-[4vw] overflow-x-scroll"
          >
            {items.map((item, i) => (
...
const SlideItem = ({ item }: { item: any }) => {
  return (
    <div className="horizontal-scroll flex flex-col items-center justify-center w-[30vw] gap-[2vh]">
      <div className="text-[1.1vw] text-yellow-400 text-nowrap font-bold">
        {item.title}
      </div>
      <img
        className="transition-all duration-400 opacity-100 rounded-xl shadow-lg aspect-[19/9] object-cover object-center w-full"
        src={item.image}
        loading="eager"
        alt=""
      />
      <div className="text-[1.2vw] normal-case">{item.description}</div>
    </div>
  );
};
